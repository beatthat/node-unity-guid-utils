...
const replaceGuids = async (opts) => {
    ...
    const readPath = opts.path_source
    const writePath = opts.path_target

    const guidMap = opts.guid_map

    if(!readPath) {
        throw new Error(`missing required param 'path_source' in call to replaceGuids with opts\n${JSON.stringify(opts, null, 2)}`)
    }

    if(!writePath) {
        throw new Error(`missing required param 'path_target' in call to replaceGuids with opts\n${JSON.stringify(opts, null, 2)}`)
    }
